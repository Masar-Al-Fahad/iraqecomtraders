...
export function ReportsPage({companies,services,can,notify}:Props){
  ...
  return <div className="space-y-4 print:bg-white report-print-root">
    <div className="print:hidden"><PageTitle title="التقارير المالية" description="كشف الشركات أو عضو واحد عبر جميع الشركات، مع إجماليات التحاسب والقبض."
      actions={<>{can('financial.reports.xlsx')&&<Button variant="outline" onClick={exportXlsx}><FileSpreadsheet className="w-4 h-4 ml-2"/>Excel حقيقي</Button>}{can('financial.reports.print')&&<Button variant="outline" onClick={()=>window.print()}><Printer className="w-4 h-4 ml-2"/>طباعة</Button>}{can('financial.reports.pdf')&&<Button variant="outline" onClick={()=>window.print()}>PDF</Button>}</>}/></div>
    <h2 className="hidden print:block text-center text-lg font-bold mb-2">التقارير المالية</h2>
    ...
    <div className="grid grid-cols-2 lg:grid-cols-6 gap-2 print:grid-cols-3 print:gap-1">{[['حجم الأعمال','gross_business_amount'],['استحقاق MFEC','mfec_due_amount'],['تم التحاسب','settled_amount'],['غير محاسب','unsettled_amount'],['المقبوض','received_amount'],['المتبقي','outstanding_receivable']].map(([label,key])=><Card key={key}><CardContent className="p-3 print:p-1"><small className="text-slate-500">{label}</small><b className="block">{money(totals[key])}</b></CardContent></Card>)}</div>
    <CompactTable printFriendly headers={['اختيار','العضو','الشركة / الخدمة','الفقرة','الكمية','حجم الأعمال','استحقاق MFEC','التحاسب','المقبوض','المتبقي']}>
