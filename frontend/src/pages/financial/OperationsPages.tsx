...
export function SettlementsPage({companies,can,notify}:Props){
  ...
  const [filter,setFilter]=useState('all');const [open,setOpen]=useState(false);const [details,setDetails]=useState<any[]>([]);
  const [form,setForm]=useState({settled_at:today(),reference_number:'',notes:'',attachment_key:''});
  useEffect(()=>{if(!companyId&&companies[0])setCompanyId(companies[0].id)},[companies]);
  const load=async()=>{if(!companyId)return;try{const [r,b]=await Promise.all([financialErpApi.report(financialErpApi.query({company_id:companyId,accounting_year:year,accounting_month:month,settlement_status:'unsettled'})),financialErpApi.settlements(companyId)]);setLines(r.items);setBatches(b.items);setSelected([])}catch(e){notify(e)}};
  ...
  return <div className="space-y-4"><PageTitle title="التسويات" description="تحاسب جماعي لأسطر كشوف معتمدة، مع دفعة مرقمة وعكس مدقق لا يحذف الأسطر."/>
    <Card><CardContent className="p-3 flex gap-2 items-end flex-wrap"><Field label="الشركة"><select className="h-10 border rounded-md px-3 min-w-56" value={companyId} onChange={e=>setCompanyId(Number(e.target.value))}>{companies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></Field><Field label="السنة"><Input className="w-28" type="number" value={year} onChange={e=>setYear(Number(e.target.value))}/></Field><Field label="الشهر"><Input className="w-24" type="number" value={month} onChange={e=>setMonth(Number(e.target.value))}/></Field><Button onClick={load}>تحميل</Button>{can('financial.settlements.create')&&<Button disabled={!selected.length} onClick={()=>setOpen(true)}><Scale className="w-4 h-4 ml-2"/>تم التحاسب ({selected.length})</Button>}</CardContent></Card>
