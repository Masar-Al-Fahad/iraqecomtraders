...
export function MemberLinksPage({companies,can,notify,finance}:Omit<Common,'services'|'reloadCompanies'>&{finance:boolean}){
  ...
  const uploadAnnex=async(file:File,replacedId?:number)=>{if(!selected)return;try{const up=await financialErpApi.upload('annexes',file);await financialErpApi.addAnnex(selected.id,{...up,original_filename:file.name,mime_type:file.type,size_bytes:file.size,signed_at:date(),replaced_id:replacedId||null});setAnnexes((await financialErpApi.annexes(selected.id)).items)}catch(e){notify(e)}};
  return <div className="space-y-4">
    ...
    <FormDialog open={open} onOpenChange={setOpen} title={selected?`حساب ${selected.member_name} لدى ${selected.company_name}`:'ارتباط عضو بشركة'} className="max-w-5xl">
      ...
      {selected&&<div className="border-t pt-4 space-y-2"><div className="flex justify-between"><h3 className="font-bold">ملحق الاتفاق الثلاثي</h3>{can('financial.annexes.manage')&&<FileButton onFile={uploadAnnex}/>}</div>{annexes.map(a=><div key={a.id} className="flex justify-between border rounded p-2"><span>{a.original_filename}</span><div className="flex"><Button size="icon" variant="ghost" onClick={()=>financialErpApi.openDocument(a.object_key)}><Eye className="w-4 h-4"/></Button>{can('financial.annexes.manage')&&<FileButton label="استبدال" onFile={file=>uploadAnnex(file,a.id)}/>} {can('financial.annexes.manage')&&<Button size="icon" variant="ghost" onClick={async()=>{await financialErpApi.deleteAnnex(selected.id,a.id);setAnnexes((await financialErpApi.annexes(selected.id)).items)}}><Trash2 className="w-4 h-4 text-red-600"/></Button>}</div></div>)}</div>}
    </FormDialog>
  </div>;
}
