...
import { CompactTable, Empty, FileButton, FormDialog, PageTitle, SearchBox, StatusBadge, money } from './FinancialUi';

type Common={companies:Company[];services:ServiceType[];reloadCompanies:()=>Promise<void>;can:(key:string)=>boolean;notify:(e:unknown)=>void};
...
export function CompaniesPage({companies,services,reloadCompanies,can,notify}:Common){
  ...
  const saveCompany=async()=>{try{
    ...
    await reloadCompanies();setCompanyOpen(false);
  }catch(e){notify(e)}};
  const savePrice=async()=>{if(!selected)return;try{
    ...
    setPricing((await financialErpApi.pricingItems(selected.id)).items);setPriceOpen(false);
  }catch(e){notify(e)}};
  const uploadAttachment=async(file:File)=>{if(!selected)return;try{
    const up=await financialErpApi.upload('contracts',file);
    await financialErpApi.addCompanyAttachment(selected.id,{...up,original_filename:file.name,mime_type:file.type,size_bytes:file.size,document_type:'contract'});
  ...
  return <div className="space-y-4">
    ...
    <FormDialog open={companyOpen} onOpenChange={setCompanyOpen} title={selected?`ملف شركة: ${selected.name}`:'إضافة شركة'} className="max-w-5xl">
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <Field label="تاريخ التعاون"><Input type="date" value={form.cooperation_started_at||''} onChange={e=>setForm({...form,cooperation_started_at:e.target.value})}/></Field>
      ...
      {(selected?can('financial.companies.edit'):can('financial.companies.create'))&&<Button onClick={saveCompany} disabled={!form.name||!form.service_type_id}>حفظ بيانات الشركة</Button>}
      ...
      {selected&&<div className="border-t pt-4 space-y-3"><div className="flex justify-between"><h3 className="font-bold">مرفقات العقود الأصلية</h3>{can('financial.contracts.manage')&&<FileButton onFile={uploadAttachment}/>}</div>
        {attachments.map(a=><div key={a.id} className="border rounded-lg p-2 flex justify-between items-center"><span><FileText className="w-4 h-4 inline ml-2"/>{a.original_filename}</span><div><Button size="icon" variant="ghost" onClick={()=>financialErpApi.openDocument(a.object_key)}><Eye className="w-4 h-4"/></Button><Button size="icon" variant="ghost" onClick={()=>financialErpApi.openDocument(a.object_key,a.original_filename)}><Download className="w-4 h-4"/></Button>{can('financial.contracts.manage')&&<Button size="icon" variant="ghost" onClick={async()=>{await financialErpApi.deleteCompanyAttachment(selected.id,a.id);setAttachments((await financialErpApi.companyAttachments(selected.id)).items)}}><Trash2 className="w-4 h-4 text-red-600"/></Button>}</div></div>)}
...
export function MemberLinksPage({companies,can,notify,finance}:Omit<Common,'services'|'reloadCompanies'>&{finance:boolean}){
  ...
  const [items,setItems]=useState<AccountItem[]>([]);const [annexes,setAnnexes]=useState<Attachment[]>([]);
  ...
  const save=async()=>{try{const payload={...form,member_id:Number(form.member_id),company_id:Number(form.company_id),started_at:form.started_at||null,ended_at:form.ended_at||null,
  ...
  const saveItems=async()=>{if(!selected)return;try{await financialErpApi.saveAccountItems(selected.id,items.map(x=>({pricing_item_id:x.pricing_item_id,unit_price_override:x.unit_price_override??null,mfec_share_type_override:x.mfec_share_type_override||null,mfec_share_value_override:x.mfec_share_value_override??null,is_active:x.is_active,started_at:null,ended_at:null,notes:null})));setItems((await financialErpApi.accountItems(selected.id)).items)}catch(e){notify(e)}};
  const uploadAnnex=async(file:File)=>{if(!selected)return;try{const up=await financialErpApi.upload('annexes',file);await financialErpApi.addAnnex(selected.id,{...up,original_filename:file.name,mime_type:file.type,size_bytes:file.size,signed_at:date()});setAnnexes((await financialErpApi.annexes(selected.id)).items)}catch(e){notify(e)}};
  return <div className="space-y-4">
    ...
    <FormDialog open={open} onOpenChange={setOpen} title={selected?`حساب ${selected.member_name} لدى ${selected.company_name}`:'ارتباط عضو بشركة'} className="max-w-5xl">
      <div className="grid md:grid-cols-3 gap-3"><Field label="العضو"><select disabled={!!selected} className="w-full h-10 border rounded-md px-3" value={form.member_id} onChange={e=>setForm({...form,member_id:e.target.value})}><option value="">اختر</option>{members.map(m=><option key={m.id} value={m.id}>{m.membership_number} · {m.member_name} · {m.business_name}</option>)}</select></Field>
        ...
        <Field label="رابط بوابة العميل" className="md:col-span-2"><Input dir="ltr" value={form.customer_portal_url||''} onChange={e=>setForm({...form,customer_portal_url:e.target.value})}/></Field><Field label="تاريخ البدء"><Input type="date" value={form.started_at||''} onChange={e=>setForm({...form,started_at:e.target.value})}/></Field>
      ...
      </div>{can(selected?'financial.member_links.edit':'financial.member_links.create')&&<Button onClick={save} disabled={!form.member_id||!form.company_id}>حفظ الحساب</Button>}
      ...
      {selected&&<div className="border-t pt-4 space-y-2"><div className="flex justify-between"><h3 className="font-bold">ملحق الاتفاق الثلاثي</h3>{can('financial.annexes.manage')&&<FileButton onFile={uploadAnnex}/>}</div>{annexes.map(a=><div key={a.id} className="flex justify-between border rounded p-2"><span>{a.original_filename}</span><div><Button size="icon" variant="ghost" onClick={()=>financialErpApi.openDocument(a.object_key)}><Eye className="w-4 h-4"/></Button>{can('financial.annexes.manage')&&<Button size="icon" variant="ghost" onClick={async()=>{await financialErpApi.deleteAnnex(selected.id,a.id);setAnnexes((await financialErpApi.annexes(selected.id)).items)}}><Trash2 className="w-4 h-4 text-red-600"/></Button>}</div></div>)}</div>}
    </FormDialog>
  </div>;
}
