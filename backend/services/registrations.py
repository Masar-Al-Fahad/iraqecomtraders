...
class RegistrationsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Registrations]:
    ...
    async def get_list(
        ...
        """Get paginated list of registrationss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Registrations, field):
                        column = getattr(Registrations, field)
            ...
            stmt = select(Registrations, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Registrations, field_name):
                        stmt = stmt.order_by(getattr(Registrations, field_name).desc())
                else:
                    if hasattr(Registrations, sort):
                        stmt = stmt.order_by(getattr(Registrations, sort))
            else:
                stmt = stmt.order_by(Registrations.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Registrations)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Registrations]:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Registrations]:
        ...
        try:
            ...
            column = getattr(Registrations, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Registrations).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
