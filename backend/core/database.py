...
class DatabaseManager:
    ...
    def _normalize_async_database_url(self, raw_url: str) -> str:
        ...
        if "+aiosqlite" in drivername or "+asyncpg" in drivername or "+aiomysql" in drivername:
            if "+asyncpg" in drivername:
                url = self._strip_libpq_ssl_query_params(url)
    ...
    @staticmethod
    def _strip_libpq_ssl_query_params(url):
        """Remove libpq sslmode/ssl query keys; asyncpg uses connect_args['ssl'] instead."""
        if not url.query:
            return url
        drop = {"sslmode", "ssl"}
        if not (drop & set(url.query)):
            return url
        return url.set(query={k: v for k, v in url.query.items() if k not in drop})

    @staticmethod
    def _needs_asyncpg_ssl(database_url: str) -> bool:
        lowered = database_url.lower()
        if "supabase.co" in lowered or "supabase.com" in lowered:
            return True
        if "ssl=require" in lowered or "sslmode=require" in lowered:
            return True
        if "ssl=true" in lowered or "sslmode=verify-full" in lowered:
            return True
        return False
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Supabase / managed Postgres require TLS.
            # asyncpg accepts ssl="require"; ssl=True uses a verifying context that
            # can fail on some Windows CA stores with Supabase pooler chains.
            if "asyncpg" in database_url and self._needs_asyncpg_ssl(settings.database_url):
                engine_kwargs.setdefault("connect_args", {})["ssl"] = "require"
                logger.info("Enabled SSL for asyncpg (ssl=require)")
